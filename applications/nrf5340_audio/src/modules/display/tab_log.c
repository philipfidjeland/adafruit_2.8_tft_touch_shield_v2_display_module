...
K_MUTEX_DEFINE(nrf5340_display_mtx); /* This is used so there are no conflicts between scheduler and
					LVGL */
...
struct backend_context display_log_context;
...
static void backend_process(const struct log_backend *const backend, union log_msg_generic *msg)
{
	...
	if (IS_ENABLED(CONFIG_LOG_TIMESTAMP)) {
	...
	case LOG_LEVEL_ERR:
		if (CONFIG_DISPLAY_LOG_TAB_LOG_LEVEL >= 1) {
			log_list_length++;
		...
		break;
	case LOG_LEVEL_WRN:
		if (CONFIG_DISPLAY_LOG_TAB_LOG_LEVEL >= 2) {
			log_list_length++;
			add_log_msg_to_log_list(tab_log, str,
	...
	case LOG_LEVEL_INF:
		if (CONFIG_DISPLAY_LOG_TAB_LOG_LEVEL >= 3) {
			log_list_length++;
			add_log_msg_to_log_list(tab_log, str,
		...
		break;
	case LOG_LEVEL_DBG:
		if (CONFIG_DISPLAY_LOG_TAB_LOG_LEVEL >= 4) {
			log_list_length++;
			add_log_msg_to_log_list(tab_log, str,
						lv_palette_lighten(LV_PALETTE_YELLOW, 5));
		}
	...
	if (log_list_length >= CONFIG_LOG_TAB_MAX_LEN) {
		oldest_label = lv_obj_get_child(tab_log, 0);
